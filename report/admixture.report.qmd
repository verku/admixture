...
ADMIXTURE v1.3.0 [@Alexander2009] was run to estimate individual- 
based ancestry and identify genetic clusters (*K*=1–4). This maximum 
likelihood approach uses a cross-validation procedure to determine 
the best number of possible genetic groups present in the dataset, 
under the assumption that individuals are unrelated.

## Results

### Data processing

We mapped 57 re-sequenced genomes to the reference genome, 
...
def k_plot(log_files):
    ...
    for file in log_files:
        K=str(file).split(".")[-2]
        with open(file, 'r') as f:  # Open file for read
...
k_plot(["results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.1.log", "results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.2.log", "results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.3.log", "results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.4.log"])
```
...
def admixture_plot(q_file, ind_file, metadata, K):
    ...
    merged_df["population"] = pd.Categorical(merged_df["population"], categories=pop_order, ordered=True)
    ...
    for i in range(K):
        ax.bar(np.arange(len(samples)), ancestry_data[i], width=bar_width, bottom=bottom, color=colors[i], edgecolor='none', label=f'Cluster {i+1}')
...
K2=admixture_plot("results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.2.Q", "results/ind_order_vcf.1980.txt", "2025_metadata.csv", 2)

K3=admixture_plot("results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.3.Q", "results/ind_order_vcf.1980.txt", "2025_metadata.csv", 3)

K4=admixture_plot("results/reference.modern.merged.biallelic.fmissing0.0.autos.1980.4.Q", "results/ind_order_vcf.1980.txt", "2025_metadata.csv", 4)
```
